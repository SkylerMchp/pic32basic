...
# 40 "src/UART.c" 2
# 1 "c:\\program files\\microchip\\xc32\\v1.42\\pic32mx\\include\\sys\\attribs.h" 1 3
# 48 "c:\\program files\\microchip\\xc32\\v1.42\\pic32mx\\include\\sys\\attribs.h" 3
...
void __attribute__((vector(171), interrupt(IPL3SOFT), nomips16)) ISR_UART4_RX (void) {
    char c;
...
void __attribute__((vector(172), interrupt(IPL3SOFT), nomips16)) ISR_UART4_TX (void) {
    if (!Q_Empty(&txq))
        U4TXREG = Q_Dequeue(&txq);
    else
        U4STAbits.UTXEN = 0;
...
char UART4_qgetc(void) {
    while (Q_Empty(&rxq))
        ;
    return Q_Dequeue(&rxq);
}

void UART4_qputc (char c) {
