...
int main( void )
{
	...
	prvSetupHardware();
#if USE_BINARY_SEMAPHORE==1
    // Create binary semaphore 
    xSemScan = xSemaphoreCreateBinary();
#else
    // Create counting semaphore starting with value of 0, max of 20
    xSemScan = xSemaphoreCreateCounting(20,0); 
#endif
    if( xSemScan == NULL )
    {  // Error - the semaphore was not created successfully.
        while (1)
            ;
    } 
    xTaskCreate(PollSwitchesTask, "Poll Switches", configMINIMAL_STACK_SIZE, NULL, 3, NULL );
    xTaskCreate(ScanLEDsTask, "Scan LEDs", configMINIMAL_STACK_SIZE, NULL, 2, NULL );
